...
static void playMusic(const char *path, float volume)
{
    ...
    snprintf(command, sizeof(command),
             "powershell -WindowStyle Hidden -NoProfile -Command \"Add-Type -AssemblyName presentationCore; $player = New-Object System.Windows.Media.MediaPlayer; $player.Open([Uri] '%s'); $player.Volume = %.2f; $player.Play(); while ($true) { Start-Sleep -Seconds 1 }\"",
             path, volume);

    ZeroMemory(&bgmProcess, sizeof(bgmProcess));
    if (!CreateProcess(NULL, command, NULL, NULL, FALSE, CREATE_NEW_PROCESS_GROUP, NULL, NULL, &si, &bgmProcess)) {
...
{
#ifdef _WIN32
    playMusic("sound\\bgm.mp3", musicVolume);
#else
    playMusic("sound/bgm.mp3", musicVolume);
#endif
}
...
void showPokemonArtByName(const char *name)
{
    char path[256];
    char line[512];
    int dexNumber = getRealDexNumber(name);
    int hasConflict = 0;
    int inMain = 0;
    FILE *fp;

    snprintf(path, sizeof(path), "tools/ascii_converter/generated/%03d.inc", dexNumber);
    fp = fopen(path, "r");
    if (fp == NULL) {
        printf("\n        [No.%03d %s]\n", dexNumber, name);
        return;
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (strstr(line, "<<<<<<<") || strstr(line, "=======")) { hasConflict = 1; break; }
    }
    rewind(fp);

    while (fgets(line, sizeof(line), fp)) {
        if (hasConflict) {
            if (strstr(line, "=======")) { inMain = 1; continue; }
            if (strstr(line, ">>>>>>>")) { inMain = 0; continue; }
            if (!inMain) continue;
        }
...
void renderBattleScreen(BattlePokemon player, BattlePokemon enemy)
{
    ...
    printf("\n");
    showPokemonArtByName(enemy.pokemon.name);
    printf("------------------------------------------------------------\n");
    showPokemonArtByName(player.pokemon.name);
