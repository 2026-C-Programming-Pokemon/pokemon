...
static int hasSubstitute(BattlePokemon *pokemon)
{
    return pokemon->substituteHp > 0;
}

static void takeSubstituteDamage(BattlePokemon *pokemon, int damage)
{
    if (damage < 1) {
        damage = 1;
    }
...
void useStatusMove(BattlePokemon *attacker, BattlePokemon *defender, Move move)
{
    ...
    if (strcmp(move.name, "변신") == 0) {
        ...
        return;
    } else if (strcmp(move.name, "대타출동") == 0) {
        if (attacker->substituteHp > 0) {
            printf("이미 대타가 있다!\n");
            return;
        }

        int cost = attacker->pokemon.hp / 4;
        if (cost < 1) {
            cost = 1;
        }

        if (attacker->currentHp <= cost) {
            printf("체력이 부족해서 대타를 세울 수 없다!\n");
            return;
        }

        attacker->currentHp -= cost;
        attacker->substituteHp = cost;
        printf("%s는 체력 %d을(를) 희생해 대타를 세웠다!\n",
               attacker->pokemon.name,
               cost);
        return;
...
int attackPokemon(BattlePokemon *attacker, BattlePokemon *defender, Move move)
{
    ...
    if (hasSubstitute(defender)) {
        printf("대타가 %s의 기술을 대신 받았다!\n", defender->pokemon.name);
        if (move.power > 0 || isFixedDamageMove(move)) {
            damage = calculateDamage(*attacker, *defender, move);
            takeSubstituteDamage(defender, damage);
        } else {
            printf("기술 효과는 대타에 의해 무효화되었다!\n");
        }
        return 0;
    }
