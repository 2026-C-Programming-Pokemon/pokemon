...
int isFixedDamageMove(Move move)
{
...
int isRecoilMove(Move move)
{
    return strcmp(move.name, "돌진") == 0 ||
           strcmp(move.name, "이판사판태클") == 0 ||
           strcmp(move.name, "지옥의바퀴") == 0 ||
           strcmp(move.name, "발버둥") == 0;
}

int isSelfDestructMove(Move move)
{
    return strcmp(move.name, "자폭") == 0 ||
           strcmp(move.name, "대폭발") == 0;
}

int isJumpKickMove(Move move)
{
    return strcmp(move.name, "점프킥") == 0 ||
           strcmp(move.name, "무릎차기") == 0;
}

int isRechargeMove(Move move)
{
...
void applyRecoilDamage(BattlePokemon *attacker, int dealtDamage)
{
    int recoilDamage;

    if (dealtDamage <= 0 || isFainted(*attacker)) {
        return;
    }

    recoilDamage = dealtDamage / 4;
    if (recoilDamage < 1) {
        recoilDamage = 1;
    }

    takeDamage(attacker, recoilDamage);
    printf("%s는 반동으로 %d의 데미지를 입었다!\n",
...
void applyJumpKickCrashDamage(BattlePokemon *attacker)
{
    int crashDamage = attacker->pokemon.hp / 2;

    if (crashDamage < 1) {
        crashDamage = 1;
    }

    takeDamage(attacker, crashDamage);
...
void applySelfDestruct(BattlePokemon *attacker)
{
...
int canPokemonMove(BattlePokemon *pokemon)
{
    if (pokemon->rechargeTurns > 0) {
        pokemon->rechargeTurns--;
        printf("%s는 반동으로 움직일 수 없다!\n", pokemon->pokemon.name);
        return 0;
    }
...
void transformPokemon(BattlePokemon *attacker, BattlePokemon *defender)
{
    int originalId = attacker->pokemon.id;
    char originalName[sizeof(attacker->pokemon.name)];
    int originalMaxHp = attacker->pokemon.hp;

    snprintf(originalName, sizeof(originalName), "%s", attacker->pokemon.name);

    attacker->pokemon = defender->pokemon;
    attacker->pokemon.id = originalId;
    snprintf(attacker->pokemon.name, sizeof(attacker->pokemon.name), "%s", originalName);
    attacker->pokemon.hp = originalMaxHp;
...
int attackPokemon(BattlePokemon *attacker, BattlePokemon *defender, Move move)
{
    ...
    if (!checkMoveHit(*attacker, *defender)) {
        ...
        if (isJumpKickMove(move)) {
            applyJumpKickCrashDamage(attacker);
        }
        return 0;
    }
    ...
    if (isRecoilMove(move)) {
        applyRecoilDamage(attacker, damage);
    }

    if (isSelfDestructMove(move)) {
        applySelfDestruct(attacker);
    }

    if (isRechargeMove(move) && !isFainted(*attacker)) {
