...
int getPokemonDexNumber(Pokemon pokemon)
{
    static const int realDexByGameId[POKEMON_COUNT + 1] = {
        0,
        3, 6, 9, 12, 15, 18, 20, 22, 24, 26,
        28, 31, 34, 36, 38, 40, 42, 45, 47, 49,
        51, 53, 55, 57, 59, 62, 65, 68, 71, 73,
        76, 78, 80, 82, 83, 85, 87, 89, 91, 94,
        95, 97, 99, 101, 103, 105, 106, 107, 108, 110,
        112, 113, 114, 115, 117, 119, 121, 122, 123, 124,
        125, 126, 127, 128, 130, 131, 132, 134, 135, 136,
        137, 139, 141, 142, 143, 144, 145, 146, 149, 150,
        151
    };

    if (pokemon.id > 0 && pokemon.id <= POKEMON_COUNT) {
        return realDexByGameId[pokemon.id];
    }
...
{
    printf("No.%03d %s\n", getPokemonDexNumber(pokemon), pokemon.name);
...
BattlePokemon createBattlePokemon(Pokemon pokemon, int level)
{
    ...
    battlePokemon.transformed = 0;
    battlePokemon.rechargeTurn = 0;
    battlePokemon.chargingTurn = 0;
    battlePokemon.semiInvulnerable = 0;
    battlePokemon.substituteHp = 0;
